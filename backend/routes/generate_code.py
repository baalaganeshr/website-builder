...
# Create stub functions for removed cloud AI services that redirect to Ollama
async def stream_claude_response(*args, **kwargs):
    """Redirect Claude calls to Ollama"""
    return await stream_ollama_response(*args, **kwargs)

async def stream_claude_response_native(*args, **kwargs):
    """Redirect Claude native calls to Ollama"""  
    return await stream_ollama_response(*args, **kwargs)
...
async def stream_gemini_response(*args, **kwargs):
    """Redirect Gemini calls to Ollama"""
    return await stream_ollama_response(*args, **kwargs)
...
class ParallelGenerationStage:
    ...
    def _create_generation_tasks(
        ...
        for index, model in enumerate(variant_models):
            if model in OPENAI_MODELS:
                ...
                tasks.append(
                    self._stream_ollama_with_error_handling(
                        prompt_messages,
                        model_name=model.value,
                        index=index,
                    )
    ...
    async def _stream_ollama_with_error_handling(
        ...
        """Wrap Ollama streaming with specific error handling"""
        try:
            # Check if Ollama is running
            is_connected = await check_ollama_connection(OLLAMA_BASE_URL)
            if not is_connected:
                error_message = (
                ...
                await self.send_message("variantError", error_message, index)
                raise VariantErrorAlreadySent(Exception(error_message))
            ...
            print(f"[VARIANT {index + 1}] Ollama error: {e}")
            error_message = (
                f"Ollama error: {str(e)}. Please check that Ollama is running and "
                f"the model '{model_name}' is available."
            )
            await self.send_message("variantError", error_message, index)
            raise VariantErrorAlreadySent(e)
